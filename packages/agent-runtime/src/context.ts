...
async function loadDocs(): Promise<{ sections: string[]; loadedDocs: string[] }> {
  const root = await resolveRootOrCwd();
...
async function resolveRootOrCwd(): Promise<string> {
  let current = process.cwd();
  for (let i = 0; i < 8; i += 1) {
    const pkg = await readText(path.join(current, "package.json"));
    const agents = await readText(path.join(current, "AGENTS.md"));
    if (agents && pkg?.includes("\"addroid-oss\"")) return current;
