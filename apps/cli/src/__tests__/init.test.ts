...
test("init はセットアップ後に使う addroid command wrapper を作成する", async () => {
  await withTempHome(async (home) => {
    const { code, out } = await capture(() => runInit([]));
    assert.equal(code, 0, out.stdout + out.stderr);

    const commandPath = path.join(home, "bin", "addroid");
    const aliasPath = path.join(home, "bin", "addroid-cli");
    const raw = fs.readFileSync(commandPath, "utf8");
    const aliasRaw = fs.readFileSync(aliasPath, "utf8");
    assert.match(raw, /^#!\/bin\/sh/);
    assert.match(raw, /Generated by addroid init/);
    assert.match(raw, /bin\/addroid\.cjs/);
    assert.equal(aliasRaw, raw);
    if (process.platform !== "win32") {
      assert.equal(fs.statSync(commandPath).mode & 0o777, 0o755);
    }
    assert.match(out.stdout, /CLI command setup:/);
  });
});

test("init は既存のユーザー管理 addroid command を上書きしない", async () => {
  await withTempHome(async (home) => {
    ...
    const { code, out } = await capture(() => runInit([]));
    assert.equal(code, 0, out.stdout + out.stderr);
    assert.equal(fs.readFileSync(commandPath, "utf8"), "#!/bin/sh\necho custom\n");
    assert.match(out.stdout, /skipped \(existing addroid command was not created by init\)/);
  });
});
...
test("init は初期設定済みでも addroid コマンド未検出なら checkout CLI link を案内して実行する", async () => {
  await withTempHome(async (home) => {
    ...
    try {
      ...
      assert.match(fs.readFileSync(path.join(home, "bin", "addroid"), "utf8"), /Generated by addroid init/);
      assert.match(out.stdout, /already initialized/);
      assert.match(out.stdout, /CLI command setup:/);
      assert.match(out.stdout, /addroid\s+: (created\/updated|available)/);
      assert.match(out.stdout, /addroid chat\s+# チャットで/);
    } finally {
      if (prevDb === undefined) delete process.env.DATABASE_URL;
      else process.env.DATABASE_URL = prevDb;
